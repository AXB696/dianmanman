<template>
  <div style="flex:1;display:flex;flex-direction:column;position:relative;margin-top:20px;overflow:hidden">
    ...
    <!-- 回到初始视角按钮 -->
    <button class="reset-view-btn" @click="resetView" title="回到初始视角">
...
// 初始视角配置
const INITIAL_CENTER = { lng: 114.305, lat: 30.593 }
...
function initMap(): void {
  ...
  map = new AMap.Map(mapRef.value, {
    zoom: INITIAL_ZOOM,
    center: new AMap.LngLat(INITIAL_CENTER.lng, INITIAL_CENTER.lat),
    viewMode: '2D',
    resizeEnable: true,
    dragEnable: true,
    zoomEnable: true,
...
function resetView(): void {
  ...
  // 平滑移动到初始位置
  map.setZoomAndCenter(INITIAL_ZOOM, new AMap.LngLat(INITIAL_CENTER.lng, INITIAL_CENTER.lat), false, 600)
}
...
async function loadStations(): Promise<void> {
  ...
  try {
    ...
    // 清除旧标记（必须在填充 stationMap 之前，否则 clearMarkers 会清空新数据）
    clearMarkers()

    for (const s of stations) {
      if (!s.location || !s.location.lat) continue
      // 过滤掉内蒙古的站点
      if (s.district_group && s.district_group.includes('内蒙古')) continue
    ...
    if (massMarks.length > 0) {
      ...
      mass.on('click', (e: any) => {
        console.log('[ChinaMap] MassMarks clicked:', e)
...
function showInfo(AMap: any, s: any, available: number, total: number, rate: number): void {
  ...
  const html = [
    ...
    'transition:color 0.2s" onmouseover="this.style.color=\'#ef4444\'" onmouseout="this.style.color=\'#9ca3af\'">✕</span>',
    '<div style="font-weight:700;color:#1677ff;margin-bottom:4px;font-size:14px;padding-right:18px">'+s.name+'</div>',
    '<div style="display:flex;gap:8px;margin-bottom:6px;font-size:10px">',
    '<span style="color:#6b7280">'+(s.district_group||'')+'</span>',
    '<span style="color:#13c2c2">'+typeName+'</span>',
    '<span style="color:#faad14">★ '+(s.rating||'-')+'</span>',
    '</div>',
    '<div style="display:flex;gap:14px;padding:6px 0;',
    'border-top:1px solid #f3f4f6;border-bottom:1px solid #f3f4f6">',
    '<div><span style="color:#9ca3af;font-size:10px">可用</span><br><span style="font-weight:700;color:#1677ff;font-size:18px">'+available+'</span></div>',
    '<div><span style="color:#9ca3af;font-size:10px">总桩</span><br><span style="font-weight:700;color:#1f2937;font-size:18px">'+total+'</span></div>',
    '<div><span style="color:#9ca3af;font-size:10px">电价</span><br><span style="font-weight:700;color:#faad14;font-size:14px">'+price+'</span></div>',
    '</div>',
    addr ? '<div style="color:#6b7280;font-size:10px;margin-top:4px">'+addr+'</div>' : '',
    tel ? '<div style="color:#6b7280;font-size:10px">📞 '+tel+'</div>' : '',
    '</div>'
  ].join('')
...
function zoomToAll(): void {
  ...
  stationMap.forEach(station => {
    if (station.location) {
      bounds.extend(new AMap.LngLat(station.location.lng, station.location.lat))
    }
  })
...
// 高亮异常站点（可用率低于30%的站点）
function highlightAbnormal(): void {
  if (!map) return
  const AMap = (window as any).AMap
  if (stationMap.size === 0) return

  // 找出异常站点
  const abnormalStations: any[] = []
  stationMap.forEach(station => {
    const avail = station.availability
    if (avail && avail.total > 0) {
      const rate = avail.available / avail.total
      if (rate < 0.3) {
        abnormalStations.push(station)
      }
    }
  })

  if (abnormalStations.length === 0) return
  ...
  // 弹出第一个异常站点的信息窗
  setTimeout(() => {
    if (abnormalStations.length > 0) {
      const station = abnormalStations[0]
      const avail = station.availability
      const total = avail.total
      const available = avail.available
      const rate = total > 0 ? available / total : 1
...
// 显示今日活跃站点（这里简化为显示所有站点）
function showActiveStations(): void {
  // 简化实现：缩放到全局视角
  zoomToAll()
}

defineExpose({ focusStation, zoomToAll, highlightAbnormal, showActiveStations, resetView })
