...
try:
    for s in stations:
        station_name_map[s["station_id"]] = s.get("name", s["station_id"])
except:
    pass
...
for i in range(600):  # 今日600条充电记录
    u = random.choice(users)
    sid = random.choice(station_ids)
    sname = station_name_map.get(sid, sid)
    hour = random.randint(0, 23)
    minute = random.randint(0, 59)
    visited = f"{today_str} {hour:02d}:{minute:02d}:{random.randint(0,59):02d}"
    duration = random.randint(15, 120)
    energy = round(random.uniform(10.0, 65.0), 1)
    cost = round(energy * random.uniform(0.8, 1.8), 2)
    cur.execute(
        "INSERT INTO history (user_id, station_id, station_name, visited_at, duration_min, cost_yuan, energy_kwh) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (u["id"], sid, sname, visited, duration, cost, energy)
    )
    history_created += 1
